...
```bash
docker compose exec toolbox python3 /app/src/extract_glossary_from_db.py --lang ja
```

The script identifies candidate glossary terms by analyzing the translation memory stored in the `app_tm` collection within ChromaDB. It produces **one CSV file per language** found in the database.

## Output

The script writes files to `/app/data/rag-analysis/` (mapped to `./data/rag-analysis/` on the host):

| File | Description |
| :--- | :--- |
| `db_derived_glossary_ja.csv` | Japanese glossary |
| `db_derived_glossary_it.csv` | Italian glossary |
| `db_derived_glossary_nl.csv` | Dutch glossary |
| ... | One per language found in the database |
...
### Key Features
* **Per-Language Output:** Generates a separate glossary CSV for each language, preventing cross-language mixing.
* **Context-Aware:** Separates identical English terms that have different Drupal `msgctxt` values (e.g., "Block" used in different UI contexts).
...
The script fetches all records from ChromaDB once, partitions them by `langcode`, then processes each language through four distinct phases.

### Phase 1: Identifying Candidates
The script scans each language's records to find potential glossary terms.
...
* **Context Separation:** The candidate key is `(source_lowercase, msgctxt)`, so the same English term with different Drupal contexts produces separate glossary entries.
...
For every candidate term identified, the script performs a frequency count across the records for that language.
...
* **Scope:** Pruning only applies within the same `msgctxt` group. Terms with different contexts are never pruned against each other.
...
The surviving terms are aggregated and exported to per-language CSV files in `/app/data/rag-analysis/`.
...
| **Context** | Drupal `msgctxt` value (empty if no context). | `Visibility` |
| **Target** | The most frequent translation (the "Winner"). | `ブラウザー` |
