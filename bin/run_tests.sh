...
#   bin/run_tests.sh --run-integration     # unit tests + integration tests (requires stack up)
...
if [[ " $* " == *" --run-integration "* ]]; then
  ...
  echo -e "  → To include integration tests: bin/run_tests.sh --run-integration"
fi
...
docker compose exec \
  -e PYTHONPATH="/dependencies:/app/src:/app/services/rag-proxy/src:/app/shared:/app/bin/lib" \
