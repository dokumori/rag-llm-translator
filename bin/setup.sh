...
section "⚙️  LLM CONNECTION MODE"
    ...
    case "$MODE_CHOICE" in
        ...
        1)
                ...
                case "$p" in
                    ...
                    custom)
                        ...
                        while true; do
                            ...
                            local_id=""
                            while true; do
                                read -p "   Local ID (e.g. amazee-ai-claude-haiku, no spaces): " local_id
                                if [ -z "$local_id" ]; then
                                    echo "   ⚠️  Local ID cannot be empty. Please try again."
                                elif [[ "$local_id" =~ [[:space:]] ]]; then
                                    echo "   ⚠️  Local ID must not contain spaces. Use hyphens instead (e.g. amazee-ai-claude-haiku)."
                                else
                                    break
                                fi
                            done
            ...
            # The OpenAI client requires a non-empty api_key even when talking to
            # LiteLLM (which has no master_key set). Any non-empty string works.
            LLM_API_TOKEN="litellm"
            ...
            # Assume we will write models.yaml; may be set to false below.
            GENERATE_MODELS=true
            if [ -f "$MODELS_YAML" ]; then
                # custom/ollama providers require user-entered model names]
                # Default to overwrite to avoid discarding them
            ...
            if [ "$GENERATE_MODELS" = true ]; then
                ...
                CUSTOM_REMOTES_STR="$(IFS='|'; echo "${CUSTOM_REMOTE_MODEL_IDS[@]+${CUSTOM_REMOTE_MODEL_IDS[*]}}")"

                # Build pipe-separated api_base_env / api_key_env strings from ENDPOINT_ENV_IDX
                CUSTOM_BASE_ENVS=""
                CUSTOM_KEY_ENVS=""
                for idx in "${ENDPOINT_ENV_IDX[@]+${ENDPOINT_ENV_IDX[@]}}"; do
                    CUSTOM_BASE_ENVS="${CUSTOM_BASE_ENVS}${CUSTOM_BASE_ENVS:+|}CUSTOM_LLM_BASE_URL_${idx}"
                    CUSTOM_KEY_ENVS="${CUSTOM_KEY_ENVS}${CUSTOM_KEY_ENVS:+|}CUSTOM_LLM_API_KEY_${idx}"
                done

                # Run inside the toolbox container where PyYAML is guaranteed available.
                ...
                docker compose exec -T toolbox python3 /app/bin/lib/model_config.py generate \
                    ...
                    --custom-displays "$CUSTOM_DISPLAYS_STR" \
                    --custom-remotes  "$CUSTOM_REMOTES_STR" \
                    --custom-base-envs "$CUSTOM_BASE_ENVS" \
            ...
            if [ -f "$LITELLM_CONFIG" ] && [ "$GENERATE_MODELS" = false ]; then
                ...
                if [[ ! "$OW_LITELLM" =~ ^[Yy]$ ]]; then
                    ...
                    GENERATE_MODELS=true  # reuse flag to trigger litellm regen
                fi
            fi
