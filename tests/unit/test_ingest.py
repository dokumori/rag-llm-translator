...
class TestIngest(unittest.TestCase):
    ...
    def test_process_glossary_deduplication(self, mock_ingest, mock_file, mock_dict_reader, mock_glob, mock_exists):
        ...
        ingest.process_glossary(mock_client, "de")
        ...
        # Arguments: client, collection_name, ids, documents, metadatas, batch_size, label
        call_args = mock_ingest.call_args[0]

        # Metadatas is at index 4 (client, collection_name, ids, documents, metadatas)
    ...
    def test_process_glossary_missing_file(self, mock_glob):
        ...
        with self.assertLogs("ingest", level="INFO") as log:
            ingest.process_glossary(
                mock_client, Path("dummy/glossary.csv"))
    ...
    def test_process_tm_logic(self, mock_ingest, mock_polib, mock_find_po, mock_exists):
        ...
        mock_client = MagicMock()
        ingest.process_tm(mock_client, "ja")

        mock_ingest.assert_called_once()

        # Verify arguments passed to existing batch ingestion logic
        # Arguments: client, collection_name, ids, documents, metadatas, ...
        call_args = mock_ingest.call_args[0]
        # arg 0 is client, arg 1 is collection_name, arg 2 is ids, arg 3 is docs, arg 4 is metadata
        ids = call_args[2]
        docs = call_args[3]
        metadata = call_args[4]
    ...
    def test_ingest_batches_incremental_skip(self):
        ...
        # Mock client.check_existing_ids to say id2 exists
        mock_client.check_existing_ids.return_value = {"id2"}
        # Mock client.add_documents to return the count of added docs
        mock_client.add_documents.return_value = 2

        ingest._ingest_batches(mock_client, "test_collection", ids, docs, meta,
                               batch_size=10, label="Test")

        # Verify add_documents was called only with id1 and id3
        mock_client.add_documents.assert_called_once()
        call_args = mock_client.add_documents.call_args[0]
        added_ids = call_args[1]  # (collection_name, ids, docs, meta)
        self.assertEqual(added_ids, ["id1", "id3"])

    def test_ingest_batches_batching(self):
        """Verifies correct batching of large datasets."""
        mock_client = MagicMock()
        mock_client.check_existing_ids.return_value = set()  # Nothing exists
        mock_client.add_documents.return_value = 4  # Approximate
        ...
        self.assertEqual(mock_client.add_documents.call_count, 3)
    ...
    def test_main_routing(self, mock_pre_flight, mock_tm, mock_gloss, mock_ingest_client, mock_args):
    ...
    @patch("ingest.process_tm")
    @patch("ingest.pre_flight_check", return_value=True)
    def test_main_reset_flow(self, mock_pre_flight, mock_tm, mock_gloss, mock_ingest_client, mock_args):
