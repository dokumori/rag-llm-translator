...
class TestTranslateRunner(unittest.TestCase):
  ...
  def test_find_po_files(self, mock_glob):
    ...
    # Verify
    mock_glob.assert_called_once_with(os.path.join('/input', "**/*.po"), recursive=True)
  ...
  @patch('translate_runner.glob.glob') # Mock glob for find_files AND temp cleanup
  ...
  def test_run_translation_workflow_isolation_and_success(self, mock_temp, mock_exec, mock_glob, mock_exists, mock_remove, mock_copy, mock_mkdirs):
    ...
    mock_copy.assert_any_call('/tmp/mock_work/file1.po', '/output/file1.po')
  ...
  def test_run_translation_workflow_missing_output(self, mock_remove, mock_mkdirs, mock_temp, mock_exec, mock_glob, mock_exists, mock_copy):
    ...
    for call_args in mock_copy.call_args_list:
        args, _ = call_args
        dest = args[1]
        self.assertNotIn('/output', dest, "Should not copy to output if file is missing")
