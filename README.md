...
- [**Cost Estimation**](docs/9_cost_estimation.md): How the pre-flight cost estimate is calculated — token heuristics, per-batch overhead (including RAG context), the output token range, and how to add pricing to `models.yaml`.